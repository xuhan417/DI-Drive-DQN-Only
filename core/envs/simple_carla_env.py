...
class SimpleCarlaEnv(gym.Env, utils.EzPickle):
    ...
    def get_observations(self) -> Dict:
        ...
        sensor_data = self._simulator.get_sensor_data()     # 'birdview' is in sensor data
        ...
        obs.update(sensor_data)         # 'birdview' is in sensor data
        obs.update(
            {
                'tick': information['tick'],                                        # 0
                'timestamp': np.float32(information['timestamp']),                  # 1
                'agent_state': navigation['agent_state'],
                'node': navigation['node'],
                'node_forward': navigation['node_forward'],
                'target': np.float32(navigation['target']),                         # 5
                'target_forward': np.float32(navigation['target_forward']),         # 6
                'command': navigation['command'],
                'speed': np.float32(state['speed']),                                # 8
                'speed_limit': np.float32(navigation['speed_limit']),               # 9
