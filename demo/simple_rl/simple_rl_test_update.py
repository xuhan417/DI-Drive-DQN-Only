...
# Configure action space (i.e., continuous or discrete)
        ...
        visualize=dict(
            type='birdview',
            outputs=['gif','video','show'],
            save_dir = '/home/xuhan/DI-Drive-Collector-Only/demo/simple_rl/default_experiment/'
        ),
        wrapper=dict(
            suite='FullTown02-v1',
        ),
    ...
    policy=dict(
        ...
        ckpt_path='/home/xuhan/DI-Drive-Collector-Only/demo/simple_rl/simple-rl/ckpt/iteration_20000.pth.tar',
        model=dict(),
        eval=dict(
            evaluator=dict(
                render=True,
...
def main(args, seed=0):
    ...
    if args.policy == 'dqn':
        ...
        carla_env = SimpleCarlaEnv(cfg.env, host, port)
